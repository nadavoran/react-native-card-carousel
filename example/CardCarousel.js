...
export class CardCarousel extends PureComponent {
    ...
    handleGestureCapture = (event, gesture) => {
        ...
        // console.log(`CarouselPan - capture: ${JSON.stringify(gesture)}`);
        this.lastIndexOffset = 0;
        this.duringScrollIndex = this.scrollIndex;
        this.scrolling = true;
        // return true;
        // this.logger.log(`handleGestureCapture ${Math.abs(gesture.dx) > 2}`);
    ...
    getCurrentOffset = () => {
        return this.tempScrollPos || this.duringScrollIndex * this.itemCalcWidth;
    };
    ...
    handleGestureMove = (event, { dx }) => {
        ...
        this.scrolling = true;
        // calc new index
        // const resolvedIndex = resolvedOffset - this.itemCalcWidth / 2 > currentOffset;

        // calc new index
        // console.time("roundFor" + dx);

        // const resolvedIndex = Math.round(
        // 	// (resolvedOffset + (dx > 0 ? -(this.itemCalcWidth / 4) : this.itemCalcWidth / 4)) / this.itemCalcWidth
        // 	resolvedOffset / this.itemCalcWidth
        // 	// (currentOffset + dx) / this.itemCalcWidth
        // );
        // // console.timeEnd("roundFor" + dx);
        //
        // if (this.scrollIndex !== resolvedIndex) {
        // 	this.scrollIndex = resolvedIndex;
        // 	this.props.onSnapToItem(resolvedIndex);
        // }
        this.cleanScrollingDebounce();
    ...
    handleGestureRelease = (event, { dx, vx }) => {
        ...
        // const resolvedIndex = Math.round(
        // 	// resolvedOffset / this.itemCalcWidth
        // 	(resolvedOffset + (dx > 0 ? -50 : 50)) / this.itemCalcWidth
        // 	// (resolvedOffset + (dx > 0 ? -(this.itemCalcWidth / 2) : this.itemCalcWidth / 2)) / this.itemCalcWidth
        // 	// (currentOffset + dx) / this.itemCalcWidth
        // );
        //
        ...
        // 	absoluteVelocity < 0.4 && deltaX < 50
        // 		? 0
        // 		: absoluteVelocity > 3 ? Math.round((absoluteVelocity - 1) * 0.2) : 1;
        //interpolated
        // //if the velocity is week, no need to jump to next far card
        // let newIndex =
        // 	dx > 0
        ...
        const absoluteVelocity = Math.abs(vx);
        // const interpolatedVelocityCards = Math.round(Math.log(absoluteVelocity) + Math.sqrt(absoluteVelocity) + 0.5);
        // const interpolatedVelocityCards = Math.round((-0.25 * (absoluteVelocity ^ 2) + 5 * absoluteVelocity) / 5);
        ...
        // dx is the finger direction, need to go add card the opposite direction
        let newIndex =
            dx > 0
        ...
        // - velocityDiff: ${velocityDifference}`);
        // console.log(`release:
        // - newIndex: ${newIndex},
        // - resIndex: ${resolvedIndex},
        // -currentOffset: ${currentOffset},
        // -resolvedOffset: ${resolvedOffset}
        // `);
        // newIndex =
        // 	newIndex > this.scrollIndex
        // 		? this.scrollIndex + 1
        // 		: newIndex < this.scrollIndex ? this.scrollIndex - 1 : newIndex;
        this.logger.log(
        ...
        let animatedListenerId = this.finishSwipeAnimation.addListener(event => {
            // this.logger.log(`finishSwipeAnimation event: ${event.value}`);
        ...
        // });
        //
        // this.setState(() => ({ currentIndex: newIndex }), () => this.props.onIndexChange(newIndex));
    };
    ...
    componentDidUpdate(prevProps, prevState) {
        // if (this.props.extraDetails !== prevState.extraDetails) {
        // 	// Animated.timing(this.outOfFocusAnimation, {
        // 	// 	useNativeDriver: true,
        // 	// 	duration: 250,
        // 	// 	toValue: this.props.extraDetails ? 1 : 0
        // 	// }).start(() => {
        // 	// 	requestAnimationFrame(() => this.runRemoveAnimation(prevState));
        // 	// });
        // } else if (!this.outOfFocus && this.state.removingCard !== prevState.removingCard) {
        // 	this.runRemoveAnimation(prevState);
        // }
    }
    ...
    _onScroll = event => {
        ...
        // 		this.reportScroll = false;
        // 		requestAnimationFrame(() =>
        // 			analyticsUtils.trackEvent("ApprovalsCarousel", "navigate", { navigation_method: "swipe_carousel" })
        // 		);
        // 	}
        // 	// requestAnimationFrame(() => {
        // 	// this.props.onSnapToItem(this.snappingToIndex);
    ...
    onTouchEnd = event => {
        this.reportScroll = false;
        // this.slideFixing = false;
        // const index = this.getIndex(event);
        // if (index !== this.onScrollIndex) {
        // requestAnimationFrame(() => this.props.onSnapToItem(this.onScrollIndex));
        // this.props.onSnapToItem(this.onScrollIndex);
        // }
    };
    ...
    renderItem = ({ item, index }) => {
        ...
        return (
            <Animated.View style={[ containerStyle, styles.cardContainer ]} pointerEvents={"box-none"}>
                ...
                {/*<Text>currentIndex: {this.scrollIndex}</Text>*/}
                {/*</View>*/}
                {Platform.select({
                    android: (
                        <Animated.View
    ...
    // renderHeader() {
    //     return (
    //         <View style={{ flex: 1, borderWidth: 1, width: this.props.itemCalcWidth - 10 }}>
    //             <Text style={{ width: this.props.itemCalcWidth - 10 }}>This is the first card</Text>
    //         </View>
    //     );
    // }
    //
    ...
    //             <Text>This is an empty Carousel</Text>
    //         </View>
    //     );
    // }
    //
    // getItemLayout = (data, index) => {
    //     // extra 20 for the padding
    //     return { length: this.itemCalcWidth, offset: index * this.itemCalcWidth, index };
    // };

    // onScrollToIndexFailed = ({ index, highestMeasuredFrameIndex, averageItemLength }) => {
    //     // extra 20 for the padding
    //     return this.flatList.scrollTo(this.itemsLength * index);
    // };
